...
public class GmsgCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 1) {
            player.sendMessage(ChatFormat.color("&cUsage: /" + label + " <group> <message>  &7(or &e/" + label + " <message> &7if you have 1 group)"));
            player.sendMessage(ChatFormat.color("&7Also: &e/" + label + " toggle [group] &7and &e/" + label + " history [group] [1-5] &7and &e/" + label + " <group> <toggle|history> [1-5]"));
            return true;
        }

        // --- toggle / history shortcuts via /gmsg and /gm (aliases share same executor) ---
        // group-first: /gmsg <group> toggle  and /gmsg <group> history [count]
        ...
        // literal-first: /gmsg toggle [group]  and /gmsg history [group] [count] or /gmsg history [count]
        ...
        String message;
        if (args.length == 1) {
            // single arg: treat as message with inferred group (only valid if exactly 1 group)
            group = null;
            message = args[0];
        } else {
            // Check if first arg looks like a group name the player has, or try infer fallback
            // If player has exactly 1 group and args[0] is not that group name, it might be start of message with omitted group.
            // We assume explicit group when they have 1+ groups and first arg matches one of their groups.
            ...
            if (args.length >= 2 && !firstIsGroup && ownGroups.size() == 1) {
                // infer group: entire args are message
                group = null;
        ...
        if (message == null || message.isBlank()) {
            player.sendMessage(ChatFormat.color("&cUsage: /" + label + " <group> <message>"));
            return true;
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 2) {
            // /gmsg toggle <group>  or  /gmsg history <group>|<count>  or  /gmsg <group> toggle|history
            if (args[0].equalsIgnoreCase("toggle")) {
                List<String> matches = new ArrayList<>();
